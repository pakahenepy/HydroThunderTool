...
- **H\*** = the per-track SCENE files (`H<track>T<name>TRH0`, one per track + menu scenes `HWTBTS_` boat-select / `HWTCRED` credits / `HWTHISC` high-score / `H_TMASS`). Partially mapped (2026-07-06):
  ...
  - Still to do: the node-type catalog (object placements for full scene reassembly), chunk descriptors/splines semantics. Entry point: the track-descriptor table in .data (`0x4ef8b8`+, referenced via `0x4eef48`/`0x4ef054`).
