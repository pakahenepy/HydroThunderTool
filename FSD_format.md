...
- **A\*** = keyframe animations for ambient props, DECODED & ENGINE-CONFIRMED (2026-07-08, loader at exe `0x408cd1`): header u16 frames + u16 bones @+4, f32 dt @+8; then per bone {u32 id, u32 key-run offset} + 108-byte bind record; then **bone-major** key runs (nframes × 108 per bone — the exe computes key base as `rec+4 + 8 + nbones×0x74`, constants at `.rdata 0x4895b8/bc`). Each 108-byte record = {3×3 matrix (column-major) + f32×3 translation, its precomputed inverse, 1.0, 1.0, 0.0} — the engine reads only block 1 (translation at +0x24, rotation converted to euler via atan2). Playback: `M(key) × inverse(M(bind))` per sub-part (identity at bind). Same declared-size undercut as P/D (split keeps slack). `hydrotool.py anims` dumps JSON; the Blender add-on plays them.
